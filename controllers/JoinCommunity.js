...
exports.registerNewUser = async function registerNewUser(newUserDataSet, callback) {
    ...
    var newUser = new userModel(newUserDataSet.username, newUserDataSet.password, timeStamp, timeStamp, "OK", "ACTIVE", "NO", "Y", "", "", "", "", 0, 0);
...
exports.userLogin = async function userLogin(username, password, choice, callback) {
    ...
    user.userLogin(username, password, function(logs) {
        if (logs.exist) {
            logs.exist = true; logs.username = username;logs.message = "The user exists"; 
            ...
            if (choice == "confirm") {
                ...
                _this.registerNewUser(user, function(err, result) {
                    if (err) {
                        logs.err = err;logs.code = 401; logs.message = "The User failed to be added";
                        callback(logs);
                    } else {
                        logs.created = true; logs.code = 201;logs.username = username;
                ...
                logs.cancel = true;logs.message = "The user cancelling the registration"; callback(logs);
            }
        }
    });
}
...
exports.updateOnlineStatusForLogout = async function updateOnlineStatusForLogout(username, callback) {
    ...
    user.updateOnlineStatusForLogout(username, function(err, numbAffected) {
        if (err) {callback(err, numbAffected);
        } else {
            var theSocketController = new SocketController(thisApp.socketio);
...
exports.addScrumb = async function addScrumb(scrumbData, callback) {
    ...
    statusCrumb1.save().then(function(feedback) {callback(false, feedback)
    }).catch(err => { callback(true, err) })
}

userModel.getUsersByCriteria("userName", "nayebare").then(function(result){})