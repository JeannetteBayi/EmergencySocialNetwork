...
module.exports.isUserExists = async function isUserExists(theUsername, callback) {
    ...
    userModel.model('User').findOne(function(err, user) {
        if (err) {
            callback(err, user);
        } else if (user) {
            callback(err, user);
...
module.exports.updateOnlineStatusForLogin = async function updateOnlineStatusForLogin(theUsername, callback) {
    ...
    userSchema.update({ username: theUsername }, { $set: { onlineStatus: 'Y', lastLogin: datetime } }, { new: true }, function(err, numAffected) {
        callback(err);
    });
}
...
module.exports.verifyUserOnline = async function verifyUserOnline(theUsername, callback) {
    ...
    userSchema.findOne({ username: theUsername, onlineStatus: 'Y' }, function(err, data) {
        callback(err, data);
    });
}

module.exports.AssignUserSocketId = async function AssignUserSocketId(user, callback) {
    userSchema.update({ username: user.username }, { $set: { socketId: user.socketid } }, options, function(err, numAffected) {
...
module.exports.updateOnlineStatusForLogout = async function updateOnlineStatusForLogout(theUsername, callback) {
    ...
    userSchema.update({ username: theUsername }, { $set: { onlineStatus: 'N' } }, { new: true }, function(err, numAffected) {
        console.log("jC log" + numAffected);
        callback(err, numAffected);
    });
...
module.exports.updateOnlineStatusForLogoutbySocketID = async function updateOnlineStatusForLogoutbySocketID(theSocketId, callback) {
    userSchema.update({ socketId: theSocketId }, { $set: { onlineStatus: 'N' } }, options, function(err, numAffected) {
        callback(err, numAffected);
    });
}
...
/*
module.exports.getUserSocketId = async function getUserSocketId(user, callback) {
    console.log("user Name we received__________________________________________", user);
    var userModel = new userSchema(user);
    var query = userModel.model('User').find();
